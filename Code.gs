...
function doPost(e) {
  var payload = {};
  try {
    // GAS receives text/plain body in postData.contents
    var raw = e.postData.contents || e.postData.getDataAsString();
    payload = JSON.parse(raw);
  } catch (err) {
    return buildResponse({ error: 'Invalid JSON: ' + err.message });
  }
  if (payload.action === 'save') {
    return buildResponse(handleSave(payload.data));
  }
  return buildResponse({ error: 'Unknown POST action' });
}

// Wrap response with CORS headers so browser fetch can read it
...
function handleLoad() {
  ...
  try {
    ...
    return { error: 'Corrupt data in RawData!B1' };
  }
}
